...
def read_input(file_path):
    ...
    for i in range(1, R + 1):
        ...
        resources.append({
            ...
            "active_turns": int(parts[3]), "remaining_life": int(parts[5])  # Track resource state
        })
...
def simulate_game(D, resources, turns):
    ...
    output_lines = []
    score = 0
    bonus_effect = {'A': 0, 'B': 0, 'C': 0, 'D': 0, 'E': 0}
    ...
    for t in range(len(turns)):
        ...
        # apply bonus effects on TR, TM, TX
        ...
        for r in active_resources[:]:  # Iterate over a copy to allow removal
            if r["active_turns"] > 0:
                ...
                if r["active_turns"] == 0:
                    r["active_turns"] = ["RW"]  # Resume operation
            r["remaining_life"] -= 1
